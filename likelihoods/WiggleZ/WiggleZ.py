...
class WiggleZ(likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        self.wigglez_a = WiggleZ_a(
            self.data_directory + self.redshift_bins_files[0], data, command_line)
        self.wigglez_b = WiggleZ_b(
            self.data_directory + self.redshift_bins_files[1], data, command_line)
        self.wigglez_c = WiggleZ_c(
            self.data_directory + self.redshift_bins_files[2], data, command_line)
        self.wigglez_d = WiggleZ_d(
            self.data_directory + self.redshift_bins_files[3], data, command_line)
