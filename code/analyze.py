...
class info:
    ...
    def __init__(self, command_line):
        ...
        # then be plotted). Initialized at empty structures.
        self.to_change = {}
        self.to_plot = []
        self.new_scales = {}
        ...
        # After this step, self.files will contain all chains.
        self.prepare(Files)
        ...
        # Create the main chain, which consists in all elements of self.spam
        ...
        # all the information will not be stored to self.files, self.covmat...
        ...
        for i in range(len(self.ref_names)):
            for j in range(i, len(self.ref_names)):
                self.covar[i, j] = np.sum(
                    chain[:, 0]*(
                        (chain[:, i+2]-self.mean[i]) *
                        (chain[:, j+2]-self.mean[j])))/weight
                if i != j:
        ...
        if command_line.plot is True:
            if command_line.comp is None:
                self.plot_triangle(
                    chain, command_line,
                    bin_number=binnumber, levels=self.lvls)
            else:
                self.plot_triangle(
                    chain, command_line, bin_number=binnumber,
                    levels=self.lvls, comp_chain=comp_chain,
        ...
        for i in indices:
            if self.scales[i, i] != 1:
                if (float(self.scales[i, i]) > 100. or
                        (self.scales[i, i]) < 0.01):
                    string = ' %0.e%s' % (
                        1./self.scales[i, i], self.ref_names[i])
        ...
        self.write_h(self.h_info, indices, 'Best Fit  ', '%.6e',
                     chain[a[0], 2:])
        self.write_h(self.h_info, indices, 'mean      ', '%.6e', self.mean)
        self.write_h(self.h_info, indices, 'sigma     ', '%.6e',
                     (self.bounds[:, 0, 1]-self.bounds[:, 0, 0])/2.)
        self.h_info.write('\n')
        self.write_h(self.h_info, indices, '1-sigma - ', '%.6e',
                     self.bounds[:, 0, 0])
        self.write_h(self.h_info, indices, '1-sigma + ', '%.6e',
                     self.bounds[:, 0, 1])
        self.write_h(self.h_info, indices, '2-sigma - ', '%.6e',
                     self.bounds[:, 1, 0])
        self.write_h(self.h_info, indices, '2-sigma + ', '%.6e',
                     self.bounds[:, 1, 1])
        self.write_h(self.h_info, indices, '3-sigma - ', '%.6e',
                     self.bounds[:, 2, 0])
        self.write_h(self.h_info, indices, '3-sigma + ', '%.6e',
                     self.bounds[:, 2, 1])
        # bounds
        ...
        self.v_info.write('%-15s\t: %-6s %-10s %-10s %-10s %-11s %-10s %-11s %-10s %-10s %-10s %-10s %-10s' % (
        ...
        for i in range(len(self.v_info_names)):
            ...
            self.v_info.write('\n%-15s\t: %.4f %.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e' % (
                name, self.R[index], chain[a[0], 2:][index],
                self.mean[index],
                ...
                self.mean[index]+self.bounds[:, 0, 0][index],
                self.mean[index]+self.bounds[:, 0, 1][index],
                self.mean[index]+self.bounds[:, 1, 0][index],
                self.mean[index]+self.bounds[:, 1, 1][index]))
    ...
    def convergence(self, is_main_chain=True, Files=None, param=None):
        ...
        for File in Files:
            ...
            for line in open(File, 'r'):
                line_count += 1
            if is_main_chain:
                self.log.write("%s\t Number of steps:%d\tSteps accepted:%d\tacc = %.2g\tmin(-loglike) = %.2f " % (
    ...
    def plot_triangle(
            self, chain, command_line, bin_number=20, scales=(),
        ...
        if comp:
            ...
            num_columns = int(round(math.sqrt(
                len(self.plotted_parameters) + len(comp_plotted_parameters))))
            ...
            num_lines = int(math.ceil(
                len(self.plotted_parameters)*1.0/num_columns))
        ...
        for i in range(len(self.plotted_parameters)):
            ...
            if plot_2d:
                ax2d = fig2d.add_subplot(
                    len(self.plotted_parameters),
                    len(self.plotted_parameters),
                    i*(len(self.plotted_parameters)+1)+1,
                    yticks=[])
            ...
            interp_hist, interp_grid = self.cubic_interpolation(
                hist, bincenters)
            ...
            if comp:
                try:
                    ...
                    interp_comp_hist, interp_comp_grid = \
                        self.cubic_interpolation(comp_hist, comp_bincenters)
            ...
            if comp_done:
                comp_bounds = self.minimum_credible_intervals(
            ...
            if plot_2d:
                ...
                ax2d.set_title('%s= $%.3g^{+%.3g}_{%.3g}$' % (
                    self.tex_names[index], self.mean[index],
            ...
            #self.get_fontsize(max(num_columns,num_lines))
            ...
            if (plot_2d and command_line.mean_likelihood):
                try:
                    ...
                    interp_lkl_mean, interp_grid = self.cubic_interpolation(
                        lkl_mean, bincenters)
            ...
            if plot_2d:
                for j in range(i):
                    second_index = self.ref_names.index(
                        self.plotted_parameters[j])
                    ax2dsub = fig2d.add_subplot(
                        len(self.plotted_parameters),
                        len(self.plotted_parameters),
                        (i)*len(self.plotted_parameters)+j+1)
                    ...
                    try:
                        cs = ax2dsub.contour(
                            ...
                            extent=extent, levels=self.ctr_level(n, lvls),
                            colors="k", zorder=5)
                    except Warning:
                    ...
                    if command_line.subplot is True:
                        ...
                        ax_temp.set_title(
                            '%s vs %s' % (
                                self.tex_names[index],
                                self.tex_names[second_index]),
                            fontsize=fontsize1d)
                        try:
                            cs = ax_temp.contour(
                                y_centers, x_centers, n, extent=extent,
                                levels=self.ctr_level(n, lvls), colors="k",
                                zorder=5)
                        except Warning:
                        ...
                        fig_temp.savefig(
                            self.folder+'plots/{0}_2d_{1}-{2}.{3}'.format(
                                self.folder.split('/')[-2],
                                self.ref_names[index],
                                self.ref_names[second_index], self.extension))

                        # store the coordinates of the points for further
                        # plotting.
                        plot_file = open(
                            self.folder+'plots/{0}_2d_{1}-{2}.dat'.format(
                                self.folder.split('/')[-2],
                                self.ref_names[index],
                                self.ref_names[second_index]), 'w')
                        plot_file.write(
        ...
        if comp:
            ...
            for i in range(
                ...
                ax1d = fig1d.add_subplot(
                    num_lines, num_columns, i+1, yticks=[])
                ii = comp_ref_names.index(
                    comp_plotted_parameters[i-len(self.plotted_parameters)])

                comp_hist, comp_bin_edges = np.histogram(
                    comp_chain[:, ii+2], bins=bin_number,
                    weights=comp_chain[:, 0], normed=False)
                comp_bincenters = 0.5*(comp_bin_edges[1:]+comp_bin_edges[:-1])
                interp_comp_hist, interp_comp_grid = self.cubic_interpolation(
                    comp_hist, comp_bincenters)
                interp_comp_hist /= np.max(interp_comp_hist)
                ...
                if command_line.subplot is True:
                    ...
                    fig1d.savefig(self.folder+'plots/{0}_{1}.{2}'.format(
                        self.folder.split('/')[-2],
                        self.ref_names[i], self.extension),
                        bbox_inches=extent1d.expanded(1.1, 1.4))
        ...
        if plot_2d:
            fig2d.savefig(
                self.folder+'plots/{0}_triangle.{1}'.format(
                    self.folder.split('/')[-2], self.extension), bbox_inches=0)
        if comp:
            fig1d.savefig(
                self.folder+'plots/{0}-vs-{1}.{2}'.format(
                    self.folder.split('/')[-2],
                    comp_folder.split('/')[-2], self.extension),
                bbox_inches=0)
        else:
            fig1d.savefig(
                self.folder+'plots/{0}_1d.{1}'.format(
                    self.folder.split('/')[-2], self.extension),
                bbox_inches=0)
    ...
    def write_tex(self, indices):
        ...
        for i in indices:
            ...
            self.tex.write("$%.4g$ & $%.4g_{%.2g}^{+%.2g}$ & $%.4g$ & $%.4g$ \\\\ \n" % (
                self.bestfit[i], self.mean[i], self.bounds[:, 0, 0][i],
                self.bounds[:, 0, 1][i], self.mean[i]+self.bounds[:, 1, 0][i],
                self.mean[i]+self.bounds[:, 1, 1][i]))
